...
Если в dApp нет функции-верификатора, то выполняется верификация по умолчанию, то есть проверка, что первое [подтверждение](/ru/blockchain/transaction/transaction-proof) содержит подпись аккаунта-отправителя. Следующая функция полностью соответствует реализации по умолчанию:
...
* В Стандартной библиотеке **версии 4** проверка первого подтверждения выполняется обязательно, вне зависимости от наличия функции-верификатора.
