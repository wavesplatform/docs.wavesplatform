...
Пользователь может оценить любой токен из браузера, в котором установлен [Waves Keeper](/ru/ecosystem/waves-keeper/), если на балансе его аккаунта не меньше 1 [WCT](/ru/blockchain/token/wct). Токены WCT при оценке не списываются, но их количество влияет на W<sub>n</sub>.
