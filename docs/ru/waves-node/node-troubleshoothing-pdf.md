...
* [Нода упала (процесс ноды внезапно завершился)](#нода-упала)
...
В данном случае подразумевается, что процесс ноды внезапно завершился (а не что-то другое - например, процесс завис, процесс не отвечает по API, процесс застрял на одной высоте). Следуя инструкциям, описанным ниже, убедитесь, что процесс именно завершился.
Если ваша нода «упала», поищите в логах [journalctl](https://www.digitalocean.com/community/tutorials/how-to-use-journalctl-to-view-and-manipulate-systemd-logs) описанные ниже или аналогичные ошибки.

Так как логи journalctl могу быть очень длинными, рекомендуется при поиске ошибок отфильтровать по дате/времени и unit'у systemd (в нашем случае waves/waves-stagenet/waves-testnet соответственно unit ноды мейннета/стейджнета/тестнета).

Для вывода лога journalctl со всеми сообщениями ноды начиная с 23 августа 2020 года 17:15:00 до текущего момента выполните следующую команду:

```bash
$ journalctl -u waves --since "2020-08-23 17:15:00"
```

Для вывода лога journalctl со всеми сообщениями ноды начиная с 23 августа 2020 года 17:15:00 до 24 августа 2020 года 03:00:00 выполните следующую команду:

```bash
$ journalctl -u waves --since "2020-08-23 17:15:00" --until "2020-08-24 03:00:00"
```

**Примечание**: Подробнее о других полезных флагах команды journalctl можно почитать тут - https://www.linode.com/docs/quick-answers/linux/how-to-use-journalctl

В journalctl проверьте есть ли в логах `systemd[1]` сообщения о том, что процесс убит/выключен/перезапустился.

**Примечание**: искать по логам можно с помощью "/поисковый_запрос" внутри journalctl или `journalctl | grep 'поисковый_запрос'`

На картинке ниже есть запись `Stopped Waves node` от `systemd` – это говорит о том, что нода действительно «упала».
Причина «падения» описана в сообщение от `systemd` - `waves.service: Main process exited, code=killed, status=6/ABRT`.

![4](_assets/node-troubleshooting-004.png)

waves[31236] – это процесс ноды. Проверьте сообщения данного процесса перед тем как нода «упала». Если в логе присутствует сообщение `An error report file with more information is saved as: /tmp/hs_err_pid*****.log`, как в примере ниже - отправьте лог команде Waves.

![5](_assets/node-troubleshooting-005.png)

Процесс `waves` может прямо указывать на ошибку, которая стала причиной «падения» ноды, как, в примере ниже:

![6](_assets/node-troubleshooting-006.png)

Также причиной «падения» ноды могут быть проблемы с памятью (OutOfMemory). Сообщения о подобных проблемах можно найти в логах `kernel`. Для вывода сообщений лога `kernel` добавьте флаг `-k` к `journalctl` как в запросе ниже:

```bash
$ journalctl -k | grep 'Kill'
```

При наличии ошибки, в выводе должны присутствовать подобные строки:
...
При выявлении подобных ошибок, отправьте фрагменты ваших логов команде Waves вместе с фрагментом waves.log, содержащим последнюю перед «падением» ноды ошибку для дальнейшего анализа.
