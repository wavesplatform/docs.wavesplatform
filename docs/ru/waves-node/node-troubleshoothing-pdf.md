...
Если ваша нода «упала», поищите в логах [journalctl](https://www.digitalocean.com/community/tutorials/how-to-use-journalctl-to-view-and-manipulate-systemd-logs) описанные ниже или аналогичные ошибки. При выявлении ошибок, отправьте фрагменты ваших логов команде Waves вместе с фрагментом waves.log, содержащим последнюю перед «падением» ноды ошибку.

Если вам нужны логи текущей загрузки (boot), запустите journalctl с помощью команды:

```bash
journalctl -b
```

Если вам нужны логи предыдущих загрузок, запустите journalctl с помощью команды:
...
Найдите необходимый boot по дате/времени и выведите его с помощью команды:

```bash
journalctl -b -2
```

Где "-2" - Boot ID, первый столбец предыдущего вывода и примера.

В journalctl проверьте есть ли в логах systemd[1] сообщения о том, что процесс убит/выключен/перезапустился. На картинке ниже запись `Stopped Waves node` от `systemd` – говорит о том, что нода действительно «упала».
Причина описана в сообщение от `systemd` - `waves.service: Main process exited, code=killed, status=6/ABRT`.
...
waves[31236] – это процесс ноды. Проверьте его сообщения перед тем как нода «упала». Если присутствует сообщение `An error report file with more information is saved as: /tmp/hs_err_pid*****.log` - отправьте лог команде Waves.

![5](_assets/node-troubleshooting-005.png)

Процесс `waves` может прямо указывать на ошибку, которая стала причиной «падения» ноды, как, например, здесь:
