...
Убедитесь, что нода работает с корректным файлом конфигурации. Подробнее про файл конфигурации в статье: https://docs.wavesprotocol.org/en/waves-node/node-configuration.
...
Сравните подпись (signature) блока на предпоследней высоте на вашей ноде и подпись блока на той же высоте на нодах из официального пула нод с помощью команды:
...
Определите последний общий блок у которого высота и подпись соответствуют такому же блоку на ноде из официального пула нод.
В логах найдите запись о первом непринятом блоке после последнего общего блока с помощью команды: 
...
Ошибка, описывающая причину непринятия блока, отображается в конце записи. 
Устранение форка необходимо выполнить в случае, если отображаются ошибки:
i. Block time 1589438183183 less than expected => эта ошибка косвенно говорит о том, что у майнера недостаточно WAVES на балансе для майнинга блока.
...
Проверьте, есть ли в syslog событие о том, что этот процесс был убит OOM-killer'ом с помощью команды dmesg | egrep -i 'killed process' (пустой вывод означает, что процесс не убит). Если процесс убит, перезапустите его с помощью команды:

sudo systemctl restart waves


Если процесс не убит OOM Killer'ом, снимите heap dump c помощью команды:
...
Если размер heap меньше чем maximum heap size, отправьте нам heap dump. Maximum heap size можно узнать из файла /etc/waves/application.ini, параметр -J-Xmx. Например: -J-Xmx2g означает, что maximum heap size = 2 Гбайт.
Если размер heap больше чем maximum heap size, перезапустить процесс с помощью команды:
...
Сразу после запуска процесса снимите heap dump с помощью команды:

sudo -u $user_запустивший_процесс jmap -dump:format=b,file=heap.bin3 $PID
Примечание: чтобы узнать PID нужного процесса, выполните команду: `pgrep -f waves`.

Найдите лог /tmp/hs_err_pid{pid}.log, где {pid} = pid процесса waves.
Отправьте нам файл c heap dump из первого шага (heap.bin3) и лог hs_err_pid{pid}.log из второго шага.
