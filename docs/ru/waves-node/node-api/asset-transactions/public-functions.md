...
```
"type" - Тип трпнзакции (3 для IssueTransaction).
...
"sender" - Адрес аккаунтра отправителя, Base58.
"senderPublicKey" - Публичный ключ аккаунта отправителя, Base58.
"name" - Имя ассета.
"description" - Описание ассета.
"quantity" - Количество неделимых единиц ассета ('lets).
"decimals" - Количкство знаков после запятой.
"reissuable" - Boolean флаг. Задает возможность/невозможность перевыпуска ассета.
"signature" - Подпись всех данных странзакции, Base58.
```
...
Сжечь заданное количество ассетов. Опубликовать в сети подписанную транзакцию зжигания ассета.
...
"quantity" - Количество неделимых есдиниц ассетов ('lets) чтобы сжечь.
"timestamp" - Временная отметка транзакции.
"signature" - Подпись всех данных странзакции, Base58.
```
...
* Подписанный запрос на перевод:
    ...
    "amount" - Количество неделимых есдиниц ассетов ('lets) к отправке.
...
Опубликовать в сети несколько подписанных транзакий передачи активов с одного адреса на другой. Текущее ограничение на размер отправляемого объекта JSON составляет 1 мегабайт.
