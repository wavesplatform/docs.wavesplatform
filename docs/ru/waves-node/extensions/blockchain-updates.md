...
Чтобы обновить расширение Blockchain Updates с предыдущих версий:

1. Заново загрузите историю изменений:

   1.1. Удалите файлы базы данных: они находятся в каталоге, указанном в настройке `waves.db.directory` (по умолчанию — в каталоге `data` основного каталога приложения.

   1.2. Импортируйте блокчейн (см. раздел [Импортировать и экспортировать блокчейн](/ru/waves-node/options-for-getting-actual-blockchain/import-from-the-blockchain)) либо дождитесь синхронизации блокчейна в обычном режиме работы ноды.

2. Скачайте новые protobuf-схемы и сгенерируйте клиентские заглушки, см. [Генерация клиента](#генерация-клиента) выше. Смигрируйте код своего приложения на новые заглушки.

Изменения в схемах не затронули большинство полей предыдущей версии. После перехода на новую версию вы можете начать использовать новые поля. Описание новых возможностей см. в [Release Notes 1.2.20](https://github.com/wavesplatform/Waves/releases/tag/v1.2.20).
