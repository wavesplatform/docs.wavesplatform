...
* Убедитесь, что у ноды достаточно соединений с другими нодами.
Для этого в файле конфигурации ноды проверьте строку с параметром [`waves.miner.quorum`](/ru/waves-node/node-configuration#настроики-генератора-блоков) (по умолчанию `quorum` = 1). Параметр позволяет задать минимальное количество подключений к другим нодам, необходимых для генерации блоков. Если нода не может подключиться к заданному количеству нод (например, из-за проблем с интернет подключением или из-за попадание в черные списки других нод), то генерация блоков будет приостановлена. Проверить текущие подключения к другим нодам можно с помощью API метода [GET /peers/connected](/ru/waves-node/node-api/peers#get-peers-connected).


это нужно, чтобы нода не уходила на форк из-за сетевых проблем. Потому что если нода потеряет связь с остальными нодами и в таком режиме намайнит в одиночку больше 100 блоков, то потом при восстановлении соединения уже не сможет автоматически вернуться со своего форка на общую цепочку
по умолчанию, нода может автоматически откатиться на общую цепочку, если форки отличаются не более чем на 100 блоков.
И можно откатить ноду вручную (запросом через апи), если форки отличаются не более чем на 2000 блоков.
