...
Наиболее удобный способ установить/обновить ноду Waves на Linux - с помощью APT-репозитория. Репозиторий предоставляет пакеты Mainnet, Testnet и Stagenet.
...
Запустите консоль, перейдите в папку с архивом `.jar` с помощью команды `cd /opt/waves` и запустите ноду с помощью следующей команды (замените {*} именем файла):
