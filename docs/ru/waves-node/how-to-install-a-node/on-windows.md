...
В этой статье объясняется, как установить ноду Waves на компьютерах под управлением Windows.
...
Установите OpenJDK 8 по [ссылке](https://access.redhat.com/documentation/en-us/openjdk/8/html/openjdk_8_for_windows_getting_started_guide/getting_started_with_openjdk_for_windows).
...
Откредактируйте файл конфигурации. **Это очень важно! Безопасность вашего кошелька и авктивов зависит от этого** Подробнее в статье [Конфигурация ноды](/en/waves-node/node-configuration).

Запустите консоль и перейдите в папку с архивом `.jar`и запустите ноду с помощью следующей команды (замените {*} именем файла):
