...
> Seed-фразу, выведенную в STDOUT, рекомендуется сохранить в надёжном месте. Если вы забудете её, то не сможете восстановить в изначальном виде. Восстановить возможно будет только хеш от seed. Чтобы получить его, потребуется сделать GET-запрос на адрес `/addresses/seed/{address}` [API ноды](/ru/waves-node/node-api.md). Хешированный seed можно использовать для [получения закрытого ключа аккаунта](/en/blockchain/waves-protocol/cryptographic-practical-details.md).
