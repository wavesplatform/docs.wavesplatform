...
> Seed-фразу, выведенную в STDOUT, рекомендуется сохранить в надёжном месте. Если вы забудете её, вы сможете восстановить захешированный с нонсом seed. Чтобы получить его, необходимо сделать GET-запрос на адрес `/addresses/seed/{address}` [API ноды](/ru/waves-node/node-api.md). Хешированный seed можно использовать для [получения закрытого ключа аккаунта](/en/blockchain/waves-protocol/cryptographic-practical-details.md). Отметим, что саму seed-фразу восстановить не получится.
