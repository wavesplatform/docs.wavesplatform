...
* Структура `stateChanges` для транзакции вызова скрипта содержит результат действий скрипта `Lease` и `LeaseCancel`. Ее возвращают следующие методы:
   * `/debug/stateChanges/address/{address}/limit/{limit}`
   * `/debug/stateChanges/info/{id}`
   * структура `stateChanges` добавлена также в ответ метода `/transactions/address/{address}/limit/{limit}` для транзакции вызова скрипта.
...
* Структура `trace` для транзакции вызова скрипта содержит результат действий скрипта `Lease` и `LeaseCancel`. Ее возвращают следующие методы:
...
Активация перечисленных выше функциональностей производится путем голосования за фичу №&nbsp;16 “Continuations”. 
