...
* Изменения для [транзакции вызова скрипта](/ru/blockchain/transaction-type/invoke-script-transaction):
   ...
   * Суммарная сложность всех вызываемых функций и скриптов ассетов в одной транзакции — не более 52&nbsp;000 (cложность скрипта отправителя не учитывается в этом лимите). 
...
* Все методы возвращают адреса вместо псевдонимов (кроме `/alias/by-address/{address}`).
