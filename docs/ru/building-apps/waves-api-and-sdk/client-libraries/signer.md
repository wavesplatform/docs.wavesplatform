...
Signer предоставляет протокол для взаимодействия с Провайдером — внешней библиотекой, которая выполняет аутентификацию пользователей и генерирует подтверждения транзакций. Ваше приложение, как и Signer, не имеет доступа к приватному ключу и SEED-фразе пользователя.

![](./_assets/signer.png)

В данный момент вы можете подключить одного из следующих провайдеров:

* [ProviderSeed](https://github.com/wavesplatform/provider-seed) разработан командой Waves и создает аккаунт пользователя из SEED-фразы. ProviderSeed можно использовать на этапе разработки и отладки приложения.
* [ProviderWeb](https://gitlab.waves.exchange/we-public/provider-web) — приложение-кошелек, разработанное Waves.Exchange, которое хранит в зашифрованном виде секретную фразу (SEED) и закрытый ключ пользователя, гарантируя, что средства пользователей защищены от хакеров и вредоносных веб-сайтов.
...
Если пользователь вошел в аккаунт, получает текущий баланс всех ассетов в портфеле.
...
Если пользователь вошел в аккаунт, получает текущий баланс спонсорских ассетов в портфеле.
...
| fee | Комиссия за транзакцию | Рассчитывается автоматически, как описано в разделе [Комиссия за транзакцию](/ru/blockchain/transaction/transaction-fee) section |
...
Примечание: в методе `broadcast` можно использовать те же опции, что и в методе [signer.broadcast](#broadcast), описание которого приведено ниже.
...
| Имя параметра | Значение по умолчанию | Описание |
| :--- | :--- | :--- |
| dApp* | | Адрес в кодировке Base58 или псевдоним (с префиксом `alias:T:`) dApp, скрипт которого нужно вызвать |
...
| options.chain | false | [Тип: boolean] Отправлять следующую транзакцию только после того, как Send the next transaction only after the previous transaction is put in the blockchain and confirmed |
| options.confirmations | -1 | Уровень подтверждения, при котором будет разрешен Promise:<br>меньше 0 – Promise разрешается, когда транзакция попадает в UTX pool<br>0 – Promise разрешается, когда блок, содержащий троанзакцию, попадает в блокчейн<br>1 – Promise разрешается, когда в блокчейн добавляется еще один блок и т. д. |
