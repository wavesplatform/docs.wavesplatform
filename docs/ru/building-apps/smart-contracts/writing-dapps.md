...
## Контекст скрипта
Контекст скрипта — это контекст, в котором могут быть определены [неизменяемые переменные](/ru/building-apps/smart-contracts/ride-language/immutable-variables-and-the-let-keyword.md), и определены функции, которые будут доступны в пределах всего dApp.
```
let someConstant = 42
func doSomething() = {
    1+1
