...
## Контекст скрипта
Контекст скрипта — это контекст, в котором могут быть определены [неизменяемые переменные](/ru/building-apps/smart-contracts/ride-language/immutable-variables-and-the-let-keyword), и определены функции, которые будут доступны в пределах всего dApp.
```
let someConstant = 42
func doSomething() = {
    1+1
...
Такая функция помечается аннотацией `@Verifier(tx)`, где `tx` — текущая транзакция, которую в данный момент функция проверяет. Доступные поля транзакции по её типу можно посмотреть на странице [Functions and Standard Libary](/en/smart-contracts/ride-language/standard-library).
