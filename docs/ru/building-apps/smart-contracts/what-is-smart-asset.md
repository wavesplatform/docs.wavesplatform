...
Любой пользователь может не только [создать](/waves-client/assets-management/issue-an-asset.md) собственный [токен](/ru/blockchain/token.md) на платформе Waves, но и наделить его определённой функциональностью, прикрепив к нему скрипт. Токен с прикреплённым скриптом называется [смарт-ассетом](/ru/blockchain/token/smart-asset.md), а прикрепляемый скрипт — [скриптом ассета](/ru/ride/script/script-types/asset-script.md). Примеры практического применения смарт-ассетов:

- **Заморозка**. [Транзакции](/ru/blockchain/transaction.md) со смарт-ассетом могут быть запрещены до (или после) момента, когда [блокчейн](/ru/blockchain/blockchain.md) наберёт определенную [высоту](/ru/blockchain/blockchain/blockchain-height.md).
- **Черные/белые списки**. Перевод смарт-ассета может быть запрещен на указанные [адреса](/ru/blockchain/account/address.md) или, наоборот, разрешен только на указанные адреса.
...
- **Мультиподпись**. Для совершения транзакции смарт-ассет может требовать [подпись](/ru/blockchain/transaction/transaction-signature.md) нескольких аккаунтов.  
...
Прикрепить скрипт к токену можно только в момент создания токена, обновить скрипт можно в любой момент. Для создания токена можно использовать как [Waves IDE](/ru/building-apps/smart-contracts/tools/waves-ide.md), так библиотеки ([WavesJ](https://github.com/wavesplatform/WavesJ), [WavesCS](https://github.com/wavesplatform/WavesCS) и другие). Скрипт ассета должен быть написан на [Ride](/ru/ride/about-ride.md) и прикреплен к токену при помощи [транзакции установки скрипта ассета](/ru/blockchain/transaction-type/set-asset-script-transaction.md). За выполнение транзакции установки скрипта ассета взимается комиссия, которая составляет 1 [WAVES](/ru/blockchain/token/waves.md).
...
Лицу, выпустившему токен, будет начисляться комиссия после совершения [транзакции перевода](/ru/blockchain/transaction-type/transfer-transaction.md) с участием его токена.
...
Чтобы сделать невозможным перевод токена, следует возвращать `false` для транзакции перевода, [транзакции массового перевода](/ru/blockchain/transaction-type/mass-transfer-transaction.md) и [транзакции обмена](/ru/blockchain/transaction-type/exchange-transaction.md).
...
Размер комиссии за транзакцию, в которой выполняется скрипт ассета, увеличена на 0,004 WAVES. Если аккаунтом является [смарт-аккаунт](/ru/blockchain/account/smart-account.md), то размер комиссии увеличивается на дополнительные 0,004 WAVES.
