...
* [Invoke Script transaction](/en/blockchain/transaction-type/invoke-script-transaction) can contain up to 10 attached payments. The total complexity for all callable functions and asset scripts involved is limited by 52,000 (the sender's account script complexity is not included in this limit). Canceled the extra fee of 0.004 WAVES for smart assets in payments and script actions.
* For all types of transactions, the extra fee of 0.004 WAVES for sending a transaction from a smart account or dApp is only required if the complexity of sender's account script or dApp script verifier function exceeds 200.
