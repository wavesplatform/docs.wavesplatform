...
**Note**: Failure to generate blocks may also be caused by problems with you wallet or node configuration. In this case the `Next attempt acc=...` messages will have wrong address.
...
You can find the records of your node generating attempts and check what node actually succeeded to generate a block in `waves.log` file. See [Logging Configuration](/en/waves-node/logging-configuration) article for details about `waves.log`.

Navigate to the folder where the `waves.log` file is located and execute the following grep command:
