...
For the node installed from the APT repository or from a DEB package, the configuration file is embedded into the package, see the [Install Node on Ubuntu](/en/waves-node/how-to-install-a-node/on-ubuntu) article. For the node installed from a JAR file, download the sample configuration file [waves-sample.conf](https://github.com/wavesplatform/Waves/blob/master/node/waves-sample.conf).
