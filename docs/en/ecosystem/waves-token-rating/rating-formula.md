...
User can rate any token from the web browser in which [Waves Keeper](/en/ecosystem/waves-keeper/) is installed if there is at least 1 [WCT](/en/blockchain/glossary#w) on users' address balance. The amount of WCT on the balance affects the weight of the user's rate W<sub>n</sub>.
