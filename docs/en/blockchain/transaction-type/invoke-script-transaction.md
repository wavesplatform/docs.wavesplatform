...
Invoke script transaction invokes the [callable function](/en/ride/functions/callable-function) of the [dApp](/en/blockchain/account/dapp). [More about dApp and script invocation](/en/building-apps/smart-contracts/what-is-a-dapp)

In addition to the dApp address, function name and arguments, the invoke script transaction can contain a payment to dApp.
...
* The invoke script transaction can result in the token issue. `K` is the number of issued assets that are not [non-fungible tokens](/en/blockchain/token/non-fungible-token).
