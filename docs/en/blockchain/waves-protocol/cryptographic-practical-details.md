...
Our network address obtained from the public key depends on the byte chainId \('T' for testnet and 'W' for mainnet\), so different networks obtained a different address for a single seed \(and hence public keys\). Creating a byte addresses described in more detail [here](/en/blockchain/binary-format.md).
...
The process is as follows: create the special bytes for signing for transaction or block, you can find it [here](/en/blockchain/binary-format.md), then create a signature using these bytes and the private key bytes.
