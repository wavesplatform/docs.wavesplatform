...
* For Standard library version 4 — 32,767 bytes.
   ...
   Exception: the size of `bodyBytes` field of [transaction structure](/en/ride/structures/transaction-structures) can be up to 165,947 bytes. You can pass this value as an argument to the `rsaVerify` и `sigVerify` [verification functions](/en/ride/functions/built-in-functions/verification-functions) but cannot concatenate with other byte arrays.
